...
export type CommentItem = {
    ...
    text: string;
    time: string;
    replies: CommentItem[]; // This is the magic recursive part!
};
...
export default function SingleReel({ reel }: SingleReelProps) {
    ...
    // Fixed: Removed the unused '_' setter
    const [isMuted] = useState<boolean>(false);
    const [isPlaying, setIsPlaying] = useState<boolean>(true);
    const [resetKey, setResetKey] = useState<number>(0);

    // Added HTMLDivElement type for safety
    ...
    const handleSendReply = () => {
        if (replyText.trim() && replyingTo) {
            const targetId = parseInt(replyingTo);
    ...
    useEffect(() => {
        // Fixed: Captured the ref value to prevent memory leaks on unmount
        ...
        return () => {
            // Fixed: Now safely unobserves the exact captured element
            if (currentContainer) {
    ...
    // Performance Fix: Wrapped in useCallback so it doesn't rebuild on every keystroke
    ...
    ), [commentsList, replyingTo, replyText]); // Added dependencies

    return (
        <>
            <div className="w-100 d-flex justify-content-center ms-4 my-3" style={{ scrollSnapAlign: "start" }} ref={containerRef}>

                {/* Changed: Added position-relative to hold the play overlay, kept all your styles */}
                <div
                    className="position-relative d-flex justify-content-center align-items-center rounded rounded-3 cursor-pointer"
                    onClick={() => setIsPlaying(prev => !prev)}
                    style={{ background: "rgba(0,0,0,1)", height: "95vh", width: 'min(85%, 400px)', aspectRatio: '9/16', overflow: 'hidden' }}
                >
                    <ReactPlayer
                        key={resetKey}
                        src={reel.videoUrl}
                    ...
                    {/* Added: Play icon overlay that appears when paused */}
                    {!isPlaying && (
                        <div className="position-absolute d-flex justify-content-center align-items-center" style={{ top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.3)' }}>
                            <i className="bi bi-play-fill text-white" style={{ fontSize: '5rem' }}></i>
                        </div>
                    )}
                </div>
